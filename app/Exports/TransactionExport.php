...
class TransactionExport implements FromCollection, WithHeadings, WithMapping, WithStyles, ShouldAutoSize
{
    ...
    {
        $query = Transaction::with(['user','bookTransactions.book','bookTransactions.bookCopy'])
            ->orderBy('trx_borrow_date', 'desc');

        if ($this->startDate && $this->endDate) {
            $query->whereBetween('trx_borrow_date', [
    ...
    public function map($trx): array
    {
        ...
        return collect($this->columns)->map(function($col) use ($trx, $items) {
            return match ($col) {
