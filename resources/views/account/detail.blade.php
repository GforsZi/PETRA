<x-app-layout>
    ...
    <div class="row g-0">
        <div class="col-12 col-md-4 d-flex justify-content-center p-3">
            <img src="{{ asset($account['usr_card_url'] ?? '/logo/user_placeholder.jpg') }}" class="rounded-circle shadow object-fit-cover" alt="Profile Image" width="200" height="200">
        </div>
        <div class="card mb-4 col-12 col-md-8">
            ...
            <div class="d-flex m-2 gap-2">
                @if (auth()->user()->usr_id != $account['usr_id'])
                    @if ($account['usr_activation'])
                        <a class="btn btn-lg btn-warning" style="cursor: pointer;" title="Nonaktifkan akun" data-bs-toggle="modal" data-bs-target="#banConfirmation{{ $account['usr_id'] }}"><i
                                class="bi bi-ban"></i></a>
                    @else
                        <a class="btn btn-lg btn-success" style="cursor: pointer;" title="Aktifasi akun" data-bs-toggle="modal" data-bs-target="#activationConfirmation{{ $account['usr_id'] }}"><i
    ...
    <div class="modal fade" id="activationConfirmation{{ $account['usr_id'] }}" data-bs-backdrop="static" data-bs-keyboard="false" tabindex="-1"
        aria-labelledby="activationConfirmation{{ $account['usr_id'] }}Label" aria-hidden="true">
        <form id="sendMessageForm" action="/system/account/{{ $account['usr_id'] }}/activate" method="post" class="modal-dialog modal-dialog-centered">
            @csrf
            @method('PUT')
            <div class="modal-content rounded-3 shadow">
                <div class="modal-body p-4 text-center">
                ...
                <div class="container d-flex mb-4 justify-content-center">
                    ...
                    <div class="form-check">
                        <input class="form-check-input @error('rl_admin') is-invalid @enderror" name="send_wa" value="1" type="checkbox" id="gridCheck1" data-bs-container="body"
                            data-bs-toggle="popover" data-bs-placement="bottom" data-bs-trigger="hover" data-bs-title="Pemberitahuan"
