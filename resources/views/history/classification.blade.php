        ...
        <x-table_data :paginator="$histories">
            ...
            @forelse ($histories as $index => $history)
                <tr class="align-middle">
                    ...
                    <td>{{ $history->ddc_code }}</td>
                    <td>{{ $history->ddc_description }}</td>
                    <td>{{ $history->ddc_deleted_at }}</td>
                    <td>
                        <div class="dropdown dropstart">
                            ...
                            <ul class="dropdown-menu">
                                <li>
                                    <a class="dropdown-item" href="{{ $page_url . '/' . $history->ddc_id . '/detail' }}">Detail</a>
                                </li>
                                <li>
                        ...
                        <div class="modal fade" id="restore{{ $histories->firstItem() + $index }}" data-bs-backdrop="static" data-bs-keyboard="false" tabindex="-1"
                            ...
                            <form action="/system/restore/{{ $history->ddc_id }}" method="post" class="modal-dialog modal-dialog-centered">
                                @csrf
                                @method('PUT')
                                <div class="modal-content rounded-3 shadow">
                                    ...
                                    <input type="hidden" value="Classification" name="model">
